...
# tau <- tau/periods*10# specified in years
ht <- rep(0.1, length(tau))
# ht <- rep(0.5, length(tau))
laggap <- tau - lag(tau)
leadgap <- lead(tau) - tau
ht <- vapply(seq_along(tau), function(x) max(laggap[x], leadgap[x], na.rm = TRUE), runif(1))

# grids <- create_xgrid_hx(data, xgrid[[1]], hx[[1]], tau, ht, 5 )
# num_points_mat(data, xgrid, hx, tau, ht )
# ugrid <- xgrid
# hu <- hx
# qgrid <- tau
# hq <- ht
...
sapply(
      ...
      function(time) sapply(
        ...
        function(maturity) get_yield_at(
          time, maturity))) %>%
  `colnames<-`( seq_len(max_qDate)/max_qDate) %>% 
  as_tibble() %>% 
  mutate(tau = (1:36)/365) %>% 
  pivot_longer(-tau, names_to = "xgrid", values_to = "yield") %>% 
  ggplot() + geom_line(aes(tau, yield)) +
  facet_wrap(~xgrid) 
```
