...
# 
calc_dbar <- function(data, xgrid, hx, rgrid, hr, tau, ht, price_slist, cf_slist, interest, units = 365) {
  ...
  if(is.vector(tau)){
    tupq_idx <- calc_tupq_idx(data, tau, ht, units)
    ...
    for(i in 1:nrow(tau)){
      if(is.vector(ht)){
        tupq_idx <- cbind(tupq_idx, calc_tupq_idx(data, tau[i,], ht, units))
        ux_window <- cbind(ux_window, calc_ux_window(data, tau[i,], ht, units))
      } else if(nrow(ht) == 1){
        tupq_idx <- cbind(tupq_idx, calc_tupq_idx(data, tau[i,], ht, units))
        ux_window <- cbind(ux_window, calc_ux_window(data, tau[i,], ht, units))
      } else if(nrow(ht) == nrow(tau)){
        tupq_idx <- cbind(tupq_idx, calc_tupq_idx(data, tau[i,], ht[i, ], units))
...
# @source Koo, B., La Vecchia, D., & Linton, O. B. (2019). Estimation of a Nonparametric model for Bond Prices from Cross-section and Time series Information. Available at SSRN3341344.
# 
calc_hhat_num <- function(data, xgrid, hx, rgrid = NULL, hr = NULL, tau, ht, tau_p = tau, htp = ht, cf_slist = NULL, interest = NULL, units = 365) {
