...
#' @description Internal function that estimates the discount function without taking into account
#' cross products of coupon payments
...
#' calc_dbar(USbonds, ugrid, hu, qgrid, hq, price, cf)
#' }
#' @export
...
#' hx<-c(15, 15 , 15) / 365
#' cf <- calc_cf_slist(USbonds)
#' calc_dbar(USbonds, ugrid, hu, xgrid, hx, cf)
#' }
...
#' @title Estimate yield function
#' @description Estimation of discount function and transformation to yield at given dates, time to maturities, and interest rates.
#' @param data A data frame; bond data to estimate discount curve from.
#' @param ugrid A length T numeric vector; the times at which the discount curve will be estimated.
#' @param hu A length T numeric vector, bandwidth parameter determining the size of the window
#' that corresponds to each time at which the discount curve is estimated,
...
#' price <- calc_price_slist(USbonds)
#' cf <- calc_cf_slist(USbonds)
#' estimate_yield(USbonds, ugrid, hu, xgrid, hq, price, cf)
#' }
#' @export
...
estimate_yield <- function(data, ugrid, hu, rgrid, hr, xgrid, hx, qgrid, hq, price_slist, cf_slist, interest, units = 365){
  ...
  for(i in 1:nrow(day_grid)){
    ...
    # Extract the xgrid and qgrid for this value of u as xgr and qgr. The dimensions of these objects are tested earlier in the code.
...
dhat_var <- function(data, ugrid, hu, xgrid, hx, perrors, dhat, cf_slist) {
  ...
  dbar_var <- left_join(dhat, dbar_var)
  dbar_var$yeild_var = dbar_var$dhat_var / (dbar_var$xg * dbar_var$dhat)^2
  dbar_var
}
