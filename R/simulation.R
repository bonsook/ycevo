...
ycevo_data <- function(
  ...
  sim_bond_meta <- function(n, arg_range_issued, random = TRUE) {
    ...
    # n <- 30
    if (random) {
      issuedate <- first_qdate -
        days(round(
          runif(n, arg_range_issued[[2]], arg_range_issued[[1]]) * 365
        ))
    } else {
      issuedate <- seq(
        first_qdate - days(round(arg_range_issued[[1]] * 365)),
  ...
  bond_meta <- tibble::tribble(
    ...
    rowwise() %>%
    mutate(
      sim = list(sim_bond_meta(.data$n, .data$arg_range_issued, random = FALSE))
    ) %>%
    ungroup() %>%
    ...
    rowwise() %>%
    mutate(
      matdate = .data$issuedate +
        if (is_wholenumber(.data$maturity)) {
          years(.data$maturity)
        } else {
          days(round(.data$maturity * 365))
        }
    ) %>%
    # couprt
    # based on bond type
  ...
  ls_row <- lapply(wd, function(wd) {
    which(bond_meta$pqdate > wd & bond_meta$issuedate <= wd)
  })
  ...
  cf <- bond_meta[unlist(ls_row), ] %>%
    ...
    mutate(
      pdint = case_when(
        .data$pqdate == .data$matdate ~ .data$couprt / 2 + 100,
  ...
  bond_yield <- cf %>%
    ...
    mutate(
      xgrid = (.data$qdate_num - min(.data$qdate_num)) /
        (max(.data$qdate_num) - min(.data$qdate_num))
    ) %>%
